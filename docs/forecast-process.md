...
| Condition | Status |
|---|---|
| exit 0 and `missing_data_filled_fraction` ≤ 0.05 | **ok** (normal forecast) |
| exit 0 and `missing_data_filled_fraction` > 0.05 | **imputed** (forecast on imputed inputs) |
| non-zero exit (fetch failure / unfillable window) | **failed** (no forecast) |
...
is the **same or better** (`ok` > `imputed` > `failed`). So:

- a later transient failure never clobbers an earlier good forecast;
- an `imputed` result is upgraded to `ok` if a later attempt gets clean data.

## 7. Page display

The job itself always succeeds (failures are handled in software), so the static
site is re-deployed every run. The page shows:

- a status **banner** — green (current), yellow (data gap / stale / heavily
  imputed), or red (error);
- the **plot** — observed ap30 history, the current 12-hour forecast with its
  MCD uncertainty band, and a vertical "now" divider.
